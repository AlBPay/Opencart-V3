...
class ModelExtensionPaymentOplata extends Model
{
    ...
    public function capture($requestData)
    {
        ...
        if ($request->capture_status != 'captured')
            throw new \Exception('Alliance bank capture status: ' . $request->capture_status);
    ...
    public function reverse($requestData)
    {
        ...
        if ($request->reverse_status != 'approved')
            throw new \Exception('Alliance bank refund status: ' . $request->reverse_status);
    ...
    public function sendToAPI($endpoint, $requestData)
    {
        ...
        $request = $this->sendCurl('https://payment.albpay.io/api/' . $endpoint, $requestData);

        if (empty($request->response) && empty($request->response->response_status))
            throw new \Exception('Unknown Alliance bank API answer.');

        if ($request->response->response_status != 'success')
            throw new \Exception("Alliance bank response status: " . $request->response->response_status); // todo mb error message
